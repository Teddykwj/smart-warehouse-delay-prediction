...
### 변경
- **3-seed 앙상블**: `ENSEMBLE_SEEDS = [42, 123, 456]` — Final CV를 3개 시드로 실행 후 평균
  - XGB: seeds 42(8.8778), 123(8.8837), 456(8.8789) → Ensemble 8.8762
  - LGB: seeds 42(8.8659), 123(8.8695), 456(8.8692) → Ensemble 8.8632
  - 분산 감소 효과: Blend CV 8.8640→8.8631
- **피처 롤백**: v25/v26 추가분(layout context 48개, lead 6개, svl 12개) 전부 제거 → v23 피처셋 정확 복원
  - 이유: v26까지 피처 추가 시마다 CV↑/Dacon↓ 패턴 반복, CV leakage 내재 확인

### 유지
- v23 피처셋 (v17 base + simple mean TE + pseudo-labeling 1라운드)
- Pseudo-labeling 재학습은 seed 42 단일 모델 사용

### 결과
- Dacon: **10.3987** (v24 10.3999 대비 **신기록** +0.0012)
- Gap: 1.5358→1.5356 (미세 개선)
- XGB weight 0.26→0.10: 앙상블로 LGB 우위가 더 명확해짐
